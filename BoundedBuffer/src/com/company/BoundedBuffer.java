...
public class BoundedBuffer {
    ...
    public synchronized void put(int v){
        try {
            while (buff.length == pos) {
                System.out.println("FULL - W8ing for an opening");
                this.wait();
            }
            System.out.println("Writing: " + v + " in position: " + pos);
            buff[pos] = v;
            pos++;
            this.notifyAll();
    ...
    public synchronized int get(){
        try{
            while (0 == pos) {
                //System.out.println("EMPTY - W8ing for something");
                this.wait();
            }
        }
        catch (InterruptedException ignored){return 1;}
        pos--;
        int r = buff[pos];
        //System.out.println("Read: " + r + " in position: " + pos);
        this.notifyAll();
        return r;
    }
